...
function addEverythingToDatabase() {
    let everything = getAllThings();
    for(let i = 21329; i < everything.length; i++) {
        firebase.firestore().collection("dbskudud").doc(i.toString()).set({
            word: everything[i],
            score: 0
        });
    }
...
/*firebase.firestore().collection("dbskudud").doc("21000").get().then((data) => {
